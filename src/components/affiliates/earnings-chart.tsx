...
export function EarningsChart() {
  ...
  return (
    <div className="flex w-full flex-col items-center px-3 py-4 sm:px-4 sm:py-6 md:px-6 md:py-8 lg:px-8 lg:py-10">
      ...
      <div className="relative w-full max-w-[600px]">
        <div className="relative w-full" style={{ paddingBottom: '70%' }}>
          ...
          {activePoint && activeData && (
            <div
              className="pointer-events-none absolute min-w-[100px] rounded-[5px] p-2 text-left backdrop-blur-sm sm:min-w-[120px] sm:p-3"
              style={{
            ...
            >
              <p className="text-[10px] font-light text-[#E0E0E0] sm:text-[12px]">Potential</p>
              <p className="text-[10px] font-light text-[#E0E0E0] sm:text-[12px]">earnings/month</p>
              <p className="mt-1 text-[12px] font-medium text-[#00FF88] sm:text-[14px]">
      ...
      {/* Trader buttons - all in one row */}
      <div className="flex w-full justify-center gap-[0.75rem] sm:gap-3 md:gap-6">
        {earningsData.map((data) => {
          const isActive = data.traders === activeTrader;
          return (
            <button
              key={data.traders}
              className={`cursor-pointer rounded-[3px] px-2 py-1.5 text-[8px] sm:rounded-[5px] sm:px-3 sm:py-2 sm:text-[11px] md:px-4 md:py-2.5 md:text-[13px] ${
              ...
              onClick={() => setActiveTrader(data.traders)}
            >
              {data.traders}
              {isActive && <span> traders</span>}
            </button>
          );
        })}
      </div>
      ...
      {/* Disclaimer */}
      <p className="mt-4 w-full max-w-[600px] text-right text-[10px] text-[#4C4F52] sm:mt-6 sm:text-[12px]">
