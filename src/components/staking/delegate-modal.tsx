      ...
      <div className="relative bg-[#0f1419] border border-gray-800 w-full max-w-md animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6">
          ...
          <div className="flex flex-col items-center mb-6">
            <div className="w-14 h-14 bg-gradient-to-br from-purple-500 to-indigo-500 flex items-center justify-center mb-4 shadow-lg shadow-purple-500/20">
              <svg className="w-7 h-7 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
              </svg>
            </div>
            <h2 className="text-xl font-bold text-white">Delegate HYPE</h2>
            <p className="text-gray-500 text-sm mt-1">Stake with a validator</p>
          </div>
          ...
          <div className="bg-[#1a2028] border border-gray-800 p-4 mb-4">
            <div className="flex items-start justify-between mb-3">
              <div>
                ...
                <p className="text-white font-medium">{validator.name || 'Unknown'}</p>
                <p className="text-gray-600 text-xs font-mono mt-0.5">{formatAddress(validator.validator)}</p>
              </div>
              {validator.isJailed ? (
                <span className="px-2 py-1 bg-red-500/10 text-red-400 text-xs border border-red-500/30">Jailed</span>
              ) : (
                <span className="px-2 py-1 bg-green-500/10 text-green-400 text-xs border border-green-500/30">Active</span>
              )}
            </div>
            <div className="grid grid-cols-2 gap-3 pt-3 border-t border-gray-800">
              <div>
                <p className="text-gray-500 text-xs">Commission</p>
                <p className="text-white text-sm font-medium">{(parseFloat(validator.commission) * 100).toFixed(1)}%</p>
              </div>
              <div>
                <p className="text-gray-500 text-xs">Total Stake</p>
                <p className="text-white text-sm font-medium">{formatCompactNumber(validator.stake)} HYPE</p>
              </div>
            </div>
          </div>
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs text-gray-500 font-medium">Amount (HYPE)</label>
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="0.00"
                className="flex-1 bg-[#1a2028] border border-gray-800 px-4 py-3 text-white text-sm font-medium focus:outline-none focus:border-teal-500/50 placeholder-gray-600"
              />
              <button
                onClick={() => setAmount(availableBalance)}
                className="px-4 py-3 text-xs font-medium text-teal-400 hover:text-teal-300 bg-[#1a2028] border border-gray-800 hover:border-teal-500/30 transition-colors"
              >
                MAX
              </button>
            </div>
